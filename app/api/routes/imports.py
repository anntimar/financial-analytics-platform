...
def get_import(batch_id: uuid.UUID, service: Service, user: CurrentUser) -> ImportBatchResponse:
    batch = service.get(batch_id)
    ensure_company_access(user, batch.company_id)
...
def list_import_issues(
    ...
    ensure_company_access(user, service.get(batch_id).company_id)
